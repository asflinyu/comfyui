...
def generate_shot_video(
    ...
    _log("info", f"开始生成分镜视频", {"shot": image_name, "duration_s": duration_s, "mode": mode, "quality": quality})
    ...
    try:
        ...
        if mode == "s2v":
            raise RuntimeError(f"Wan S2V 失败，不回退静帧垫片: {e}") from e
        _log("warn", "ComfyUI Wan 视频失败，使用 ffmpeg fallback", {"error": str(e)})

    # Fallback: ffmpeg image+audio video（仅 I2V 等非口型流程）
...
def generate_project_videos(
    ...
    utils.debug_break()
    _log("info", "开始批量视频生成", {"shot_count": len(shots), "width": width, "height": height, "mode": mode, "quality": quality})
    shot_videos: list[dict[str, Any]] = []
    video_paths: list[Path] = []
    with GPU.hold(_log, kind="video"):

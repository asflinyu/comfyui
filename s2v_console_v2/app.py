...
from services import comfy_image, comfy_video, deepseek, fast_storyboard, talking_musetalk as talking_fast, tts, video_gen
...
def _nocache_html(name: str) -> HTMLResponse:
...
def api_index_html() -> HTMLResponse:
    return _nocache_html("index.html")


@app.get("/static/fast.html", response_class=HTMLResponse)
def api_fast_html() -> HTMLResponse:
    return _nocache_html("fast.html")
...
@app.get("/fast")
def fast_root(request: Request) -> RedirectResponse:
    return _html_redirect("/static/fast.html", request)
...
def api_health() -> dict[str, Any]:
    return {
        ...
        "fast": {
            "image_workflow": "qwen_t2i",
            "resolution": "final_720",
            "quality": "fast",
            "label": "Qwen 4步出图 + Wan fp8 4步 I2V · 720p",
        },
    }
...
class FastStoryboardReq(BaseModel):
    brief: str = ""
    mode: str = "i2v"
    project: dict[str, Any] | None = None


@app.post("/api/storyboard/fast")
def api_storyboard_fast(req: FastStoryboardReq) -> dict[str, Any]:
    """Excel 文案结构直接成分镜，不调用 LLM。"""
    if not req.brief.strip():
        raise HTTPException(400, "请先粘贴文案或上传 Excel")
    scripts = fast_storyboard.parse_script_text(req.brief)
    if not scripts:
        raise HTTPException(400, "没有解析到镜头，请用模板：镜头N（0-2s）：画面：...。台词：...。反转：...")
    project = fast_storyboard.build_project(scripts[0], mode=req.mode or "i2v", project=req.project)
    _save_project(project)
    return project


@app.post("/api/storyboard/from_excel")
def api_storyboard_from_excel(
    ...
    for f in files:
        ...
        if suffix not in (".xlsx", ".xls"):
            errors.append(f"{f.filename}: 仅支持 .xlsx/.xls")
            continue
        try:
            scripts.extend(fast_storyboard.parse_excel_bytes(f.file.read()))
        except Exception as e:
            errors.append(f"{f.filename}: {e}")
    if not scripts:
        raise HTTPException(400, "；".join(errors) or "Excel 里没有解析到镜头")
    project = fast_storyboard.build_project(scripts[0], mode=mode or "i2v")
    _save_project(project)
    if errors:
        project["parse_warnings"] = errors
    return project
